...
public class BPP {
  ...
  /** If the bp.debug property is a non-emtpy string, this
      is initialized to true. */
  public static boolean DEBUG;
  static { 
    try { DEBUG=System.getProperty("bp.debug").length() > 0; }
  ...
  int mode='\"';
  int minStage=0;
  int maxLineLength=BPPFilter.DEFAULT_MAX_LINE_LENGTH;
  List bpArgs = new ArrayList();

  /**
     <p>Command-line interface for the preprocessor.  See <a href="../../article.html">this article on the fundamentals of using BPP</a>.</p>
     ...
     Files with a <tt>.bpp</tt> exension will produce <tt>.bsh</tt> extensions, and <tt>.jpp</tt> extensions will produce <tt>.java</tt> extensions.
     </p>
     </ul>
     ...
     <td>Quote exact by default.</td>
     </tr>
     <tr>
     <td valign="top"><b>-qq</b></td>
     <td>Quote magically by default.</td>
     </tr>
     <tr>
     ...
     <li><b>after</b> The root namespace for the post-processor of this preprocessor stage (defined only if it exists).</li>
     <li><b>before</b> The root namespace for the pre-processor of this preprocessor (defined only if it exists).</li>
     </ul>
     </ul>
  */
    ...
    for (int i=0; i<args.length; ++i) {
      ...
      if (args[i].substring(0,2).equals("-D")) {
        ...
        if (eq >= 0) {
          ...
          if (DEBUG) {
            System.err.println("-D" + name + "=" + value);
          }
  ...
  private void process(String _ifile) throws Exception {
    ...
    BPPFilter f = new BPPFilter(maxLineLength);
    f.setSource(ifile);
    ...
    if (DEBUG) {
	    ...
	    System.err.println("BPP: args=" + Format.join(',',fargs));
    }
    f.filter(in,out);
    in.close();
    out.close();
